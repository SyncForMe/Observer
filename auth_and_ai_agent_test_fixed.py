...
def test_ai_agent_generation():
    ...
    ai_generate_test, ai_generate_response = run_test(
        ...
        auth=True,
        measure_time=True,
        expected_keys=["id", "name", "archetype", "background", "expertise"]
    )
    
    if ai_generate_test and ai_generate_response:
        print("✅ AI agent generation endpoint is accessible and authenticated")
        
        # The response IS the agent data directly
        agent_data = ai_generate_response
    ...
    if ai_generate_test_2 and ai_generate_response_2:
        agent_data_2 = ai_generate_response_2
        ...
        if ai_generate_response.get("name") != agent_data_2.get("name"):
