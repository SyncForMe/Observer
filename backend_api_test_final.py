...
def test_agent_management():
    ...
    if get_agents_test:
        print(f"✅ Successfully retrieved {len(get_agents_response) if get_agents_response else 0} agents")
    ...
    delete_agent_test = False  # Initialize to False
    ...
    if created_agent_ids:
        ...
        if delete_agent_test and delete_agent_response:
            ...
            # Verify the agent was deleted by trying to get it
            ...
            if get_deleted_test:
                # Check if the deleted agent ID is in the response
                agent_ids = [agent.get("id") for agent in get_deleted_response] if get_deleted_response else []
                if agent_id not in agent_ids:
                    print("✅ Agent was correctly deleted from the database")
                else:
                    print("❌ Agent still exists in the database after deletion")
            else:
                print("❌ Failed to verify agent deletion")
        else:
...
def test_simulation_control():
    ...
    if pause_sim_test and pause_sim_response:
        ...
        is_active = pause_sim_response.get("is_active", True)
        if not is_active:
    ...
    if resume_sim_test and resume_sim_response:
        ...
        is_active = resume_sim_response.get("is_active", False)
        if is_active:
            print("✅ Simulation state is correctly set to active")
        else:
...
def test_scenario_management():
    ...
    if set_scenario_test and set_scenario_response:
        ...
        scenario = set_scenario_response.get("scenario", "")
        if scenario == custom_scenario["scenario"]:
            print("✅ Custom scenario was correctly set")
        else:
            print("❌ Custom scenario was not correctly set")
    else:
    ...
    return random_scenario_test and set_scenario_test
...
def test_conversation_endpoints():
    ...
    return get_conversations_test and generate_conversation_test
