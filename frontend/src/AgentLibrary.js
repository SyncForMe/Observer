              ...
              {selectedMyAgent ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 agent-grid">
                    {quickTeams[selectedQuickTeam].agents.map((agent, index) => (
                      <div key={`${selectedQuickTeam}-${index}`} className="bg-white border border-gray-200 rounded-lg hover:shadow-md transition-shadow relative">
                        ...
                        {/* Star icon for saving to favorites */}
                        <div className="absolute top-2 right-2 z-10">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              // For now, show a message that this agent needs to be saved first
                              alert('Add this agent to your "My Agents" collection first by using it in a simulation, then you can mark it as favorite');
                            }}
                            className="bg-white/90 hover:bg-white text-gray-600 hover:text-yellow-500 w-8 h-8 rounded-full flex items-center justify-center transition-colors shadow-md"
                            title="Add to favorites (must be saved first)"
                          >
                            ☆
                          </button>
                        </div>
                ...
                <div>
                  ...
                  {favoriteAgents.length > 0 ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 agent-grid">
                      {favoriteAgents.map((agent) => (
                        <div key={agent.id} className="bg-white border border-yellow-200 rounded-lg hover:shadow-md transition-shadow relative bg-yellow-50">
                          {/* Star in top right */}
                          <div className="absolute top-2 right-2 z-10">
                            <span className="text-lg">⭐</span>
                          </div>
                          ...
                          <div className="p-4">
                            ...
                            <div className="mt-3 flex justify-between items-center">
                              <button
                                onClick={() => handleUseSavedAgent(agent)}
                                className="bg-yellow-600 hover:bg-yellow-700 text-white text-xs px-3 py-1.5 rounded-md transition-colors font-medium"
                              >
                                Add to Simulation
                              </button>
                              <button
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm mt-1">Click the star ⭐ to add favorites.</p>
                    </div>
                  )}
                </div>
              ) : selectedSector === 'myagents' ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 agent-grid">
                      {sectors[selectedSector].categories[selectedCategory].agents.map((agent) => (
                        <div key={agent.id} className="bg-white border border-gray-200 rounded-lg hover:shadow-md transition-shadow relative">
                          ...
                          {/* Star icon for favorites - visible when agent is not added */}
                          {!addedAgents.has(agent.id) && (
                            <div className="absolute top-2 right-2 z-10">
                              <button
                                onClick={(e) => {
                                ...
                                className="bg-white/90 hover:bg-white text-gray-400 hover:text-yellow-500 w-8 h-8 rounded-full flex items-center justify-center transition-colors shadow-md border border-gray-200"
                                title="Add to favorites (must add to simulation first)"
                              >
                                ☆
                              </button>
                            </div>
