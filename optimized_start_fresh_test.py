#!/usr/bin/env python3
"""
Comprehensive test script for the Optimized Start Fresh functionality.
Tests the performance improvements and reliability of the POST /api/simulation/reset endpoint.
"""
...
def generate_conversations(token, count=3):
    ...
    for i in range(count):
        try:
            response = requests.post(f"{API_URL}/conversation/generate", headers=headers, timeout=15)
            if response.status_code == 200:
                conversation = response.json()
                generated_conversations.append(conversation)
                time.sleep(1)  # Brief pause between generations
            else:
                log_test(f"Generate Conversation {i+1}", False, f"Status: {response.status_code}")
                return []
        except Exception as e:
            log_test(f"Generate Conversation {i+1}", False, f"Exception: {str(e)}")
            return []
    
    log_test("Generate Conversations", True, f"Generated {len(generated_conversations)} conversations")
    return generated_conversations

def send_observer_messages(token, count=3):
    ...
    observer_messages = [
        ...
        "I'd like everyone to share their main concerns about the timeline."
    ]
    ...
    for i in range(min(count, len(observer_messages))):
        try:
            ...
            response = requests.post(f"{API_URL}/observer/send-message", json=message_data, headers=headers, timeout=15)
            if response.status_code == 200:
...
def run_complete_test_iteration(iteration_num):
    ...
    conversations = generate_conversations(token, 3)
    if len(conversations) < 2:
        return False, 0
    
    observer_messages = send_observer_messages(token, 3)
    if len(observer_messages) < 2:
        return False, 0
...
def main():
    ...
    print(f"Target: Response time < 5 seconds, Complete data clearing")
    print("=" * 60)
    ...
    # Overall assessment
    overall_success = (successful_iterations == iterations and 
    ...
    if overall_success:
        print("   The optimized Start Fresh functionality is working perfectly!")
        ...
        if successful_iterations < iterations:
